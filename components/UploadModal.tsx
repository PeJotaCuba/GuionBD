...
export const UploadModal: React.FC<UploadModalProps> = ({ isOpen, onClose, onSave, targetStatus }) => {
  ...
  const processFile = async () => {
    ...
    try {
      ...
      const rawEntries = text.split(/_{4,}/); // Split by 4 or more underscores
      const total = rawEntries.length;
      const parsedScripts: Script[] = [];

      // Process in chunks to enable progress bar
      const chunkSize = 20;
      for (let i = 0; i < total; i += chunkSize) {
        const chunk = rawEntries.slice(i, i + chunkSize);
        
        chunk.forEach(raw => {
          const script = parseRawEntry(raw, targetStatus);
  ...
  return (
    <>
      <LoadingOverlay isVisible={isProcessing} progress={progress} message="Analizando guiones..." />
