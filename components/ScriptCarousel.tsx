...
// Función auxiliar para determinar el estilo basado en el género/programa
    ...
    <div className="mb-8 animate-fade-in relative group/carousel">
      <div className="flex items-center justify-between mb-3 px-1">
        ...
        <div className="flex gap-1.5 opacity-0 group-hover/carousel:opacity-100 transition-opacity duration-300">
          <button onClick={() => scroll('left')} className="p-1 rounded-full bg-white dark:bg-slate-800 shadow-md border border-slate-100 dark:border-slate-700 hover:bg-indigo-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors">
            <ChevronLeft size={16} />
          </button>
          <button onClick={() => scroll('right')} className="p-1 rounded-full bg-white dark:bg-slate-800 shadow-md border border-slate-100 dark:border-slate-700 hover:bg-indigo-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors">
            ...
            return (
              <div key={script.id} className="snap-start shrink-0 w-44 bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 border border-slate-100 dark:border-slate-800 group">
                ...
                <div className="p-3 flex flex-col h-24 justify-between">
                   <div>
                      <h4 className="text-xs font-semibold text-slate-800 dark:text-slate-100 leading-snug line-clamp-3" title={script.title}>
                        {script.title}
                      </h4>
                   </div>
